...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (categorySlug) {
      const foundCategory = await categoryRepository.getBySlug(categorySlug, { locale })
      if (!foundCategory) {
    ...
    const products = await productRepository.getAll({ categoryId, limit, isAvailable: true, locale })
