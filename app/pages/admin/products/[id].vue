<template>
  <div class="space-y-6 sm:space-y-8 pb-12 max-w-4xl mx-auto">
    ...
    <div>
      ...
      <p class="text-sm text-[#6B7280] mt-1">Perbarui rincian informasi, media, dan tautan e-commerce produk.</p>
    </div>

    <!-- Spinner Loading Data Awal -->
    <div v-if="isLoadingData" class="flex flex-col items-center justify-center p-16">
      <UIcon name="i-heroicons-arrow-path" class="animate-spin text-4xl text-[#C65A3A] mb-3" />
      <p class="text-sm font-medium text-[#6B7280]">Memuat data produk...</p>
    </div>
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col">
        ...
        <div class="p-5 sm:p-8 space-y-6">
          <div class="border-b border-[#E7E1D8] pb-4 mb-2">
            ...
            <p class="text-xs sm:text-sm text-[#6B7280]">Identitas utama produk dalam Bahasa Indonesia dan Bahasa Inggris (opsional).</p>
          </div>
        ...
        <div class="p-5 sm:p-8 border-t border-[#E7E1D8] space-y-6 bg-[#F7F6F2]/40">
          ...
          <UFormField label="Galeri Gambar Tambahan" description="Opsional. Foto pendukung kemasan, varian, atau angle produk lainnya." :ui="adminFormFieldUi">
            <div class="flex flex-col gap-4">
              ...
              <div v-if="form.extraImages.length > 0 || extraImageFiles.length > 0" class="flex flex-wrap gap-3">
                <!-- Existing Images -->
                <div 
                ...
                >
                  ...
                  <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <UButton color="error" variant="soft" icon="i-heroicons-trash" class="bg-white/90 hover:bg-white text-rose-600 rounded-xl cursor-pointer" @click="form.extraImages.splice(idx, 1)" />
                  </div>
                </div>
                <!-- New Upload Preview Images -->
                <div 
  ...
  try {
    ...
    if (data) {
      form.value = {
        ...
        highlightsEn: data.highlightsEn ? data.highlightsEn.join(', ') : '',
