<template>
  <div class="h-full min-h-[578px] flex flex-col bg-white rounded-[12px] overflow-hidden border border-black/10 shadow-xs group">
    <NuxtLink :to="localePath(`/products/${slug}`)" class="w-full h-71.5 shrink-0">
    ...
    <div class="w-full grow p-6 flex flex-col justify-between gap-4">
      <div class="flex flex-col gap-2">
        <NuxtLink :to="category?.slug ? localePath(`/products?category=${category.slug}#${category.slug}`) : localePath('/products')" class="bg-accent px-3 py-1 w-fit rounded-full text-xs font-medium flex items-center justify-center text-darkprimary hover:opacity-90 transition-opacity">{{ category?.name || $t('common.uncategorized') }}</NuxtLink>
        <NuxtLink :to="localePath(`/products/${slug}`)" class="font-sans text-secondary font-semibold text-2xl transition-all duration-150 hover:text-secondary/80">{{ name }}</NuxtLink>
      ...
      <div class="flex flex-col gap-3 pt-4 border-t border-t-gray-200">
        ...
        <NuxtLink :to="localePath(`/products/${slug}`)" class="w-full py-2 bg-primary flex items-center justify-center rounded-[8px] transition-all duration-150 hover:bg-primary/90">
